...
<body class="id">
  ...
  <div class="collapse navbar-collapse" id="navbarNavDropdown">
    ...
    <section>
        ...
        <select name="" id="filter" class="sort">
            <option value="">FILTER BY</option>
            <option value="7">LAST WEEK</option>
            <option value="30">LAST MONTH</option>
        </select>
        <h2 class="text-center mt-5">ALL RIDES</h2>
...
$(document).ready(function(){
    function fatch_all_ride() {
    var action='all_ride_data';
    $.ajax({
                url : 'ajaxaction.php',
                type : "POST",
                dataType :'json',
        ...
        $('#filter').change(function(){
            var filter_time = $(this).val();
            var action = 'filter_ride';
            $.ajax({
                url:'ajaxaction.php',
                type:'post',
                dataType : "json",
                data : {ride:filter_time, action:action},
                success : function(data) {
                    var output;
                for(var i=0;i<data.length;i++) { 
                    output+= "<tr><td>" + data[i]['rideid'] + "</td><td>" + data[i]['ridedate'] + "</td><td>" + data[i]['from'] + "</td><td>" + data[i]['to'] + "</td><td>" + data[i]['cabtype'] + "</td><td>" + data[i]['totaldist'] + "</td><td>" + data[i]['luggage'] + "</td><td>" + data[i]['totalfare'] + "</td><td>" + data[i]['userid'] + "</td><td>" + data[i]['status'] + "</td><td><input type='submit' class='btn btn-danger' value='DELETE' id='delbtn' data-did=" + data[i]['rideid'] + "></td></tr>";                        
